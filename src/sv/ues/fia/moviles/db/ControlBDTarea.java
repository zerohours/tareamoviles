...
public class ControlBDTarea {
	...
	public String eliminar(DetalleCategoria detallecategoria) {
		String regAfectados = "filas afectadas= ";
		int contador = 0;
		if (verificarIntegridad(detallecategoria, 1001)) {
				contador += db.delete("detcat",
					"id_categoria='" + detallecategoria.getId_categoria() + "'", null);
	...
	public String eliminarIdPregunta(DetalleCategoria detallecategoria) {
		String regAfectados = "filas afectadas= ";
		int contador = 0;
		if (verificarIntegridad(detallecategoria, 1002)) {
				contador += db.delete("detcat",
					"id_pregunta='" + detallecategoria.getId_pregunta() + "'", null);
	...
	public ArrayList<BeanPregunta> consultarCategoriaAllById(String categorias) {// verifcar
		ArrayList<BeanPregunta> lista=new ArrayList<BeanPregunta>();
		abrir();	
		...
		if (cursor != null && cursor.getCount() > 0) {
			...
			while (cursor.moveToNext()) {
				...
				String pre=cursor.getString(1);
				lista.add(new BeanPregunta(id,pre));
	...
	private boolean verificarIntegridad(Object dato, int relacion)
		...
		case 1001: {
			DetalleCategoria detcat = (DetalleCategoria) dato;
			
			String[] idcatego = { String.valueOf(detcat.getId_categoria()) };
			abrir();
			Cursor cursor = db.query("detcat", null, "id_categoria = ?", idcatego,
					null, null, null);
			if (cursor.moveToFirst()) {
				// // Se encontraron datos
				return true;
		...
		case 1002: {
			...
			String[] idcatego = { String.valueOf(detcat.getId_pregunta()) };
			abrir();
			Cursor cursor = db.query("detcat", null, "id_pregunta = ?",idcatego,
					null, null, null);
			if (cursor.moveToFirst()) {
				// // Se encontraron datos
				return true;
